...
PVOID ManualMap(HANDLE hProcess, CONST WCHAR* ModulePath)
{
	...
	((_NtCreateSection)APICall(ntdll, APICall_NtCreateSection))(&hSection, MAXIMUM_ALLOWED, NULL, &SectionSize, PAGE_READONLY, SEC_COMMIT | SEC_NO_CHANGE, hFile);
...
VOID AddSection(HANDLE hProcess, HANDLE hSection, DWORD64 CRC)
{
	...
	else
	{
		if (hProcess != CURRENT_PROCESS)
			((_NtDuplicateObject)APICall(ntdll, APICall_NtDuplicateObject))(CURRENT_PROCESS, hSection, hProcess, &hSection, MAXIMUM_ALLOWED, NULL, DUPLICATE_SAME_ACCESS);
